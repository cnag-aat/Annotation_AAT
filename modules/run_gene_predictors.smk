...
if config["Parameters"]["run_augustus"] or config["Parameters"]["run_geneid"] or config["Parameters"]["run_geneid_introns"] or config["Parameters"]["run_augustus_hints"]:
  if masked_chunks > 1:
   ...
   # print (dirMaskedChunks + name_masked)
    ...
    if config["Parameters"]["run_geneid"]:
      use rule geneid_jobarray from gene_predictors_workflow with:
        input:
          fasta = dirMaskedChunks + name_masked + "." + str(masked_chunks) + ".fa",
          geneid_parameters = config["Inputs"]["geneid_parameters"]
        output:
          touch = dirGeneidOutput + "/geneid_run.ok"
        params:
          prefix = dirMaskedChunks + name_masked,
    ...
    if config["Parameters"]["run_geneid_introns"]:
      use rule geneid_introns_jobarray from gene_predictors_workflow with:
        input:
          fasta = dirMaskedChunks + name_masked + "." + str(masked_chunks) + ".fa",
          geneid_parameters = config["Inputs"]["geneid_parameters"],
          junctions = config["Outputs"]["output_dir"] + "coding_junctions.sorted.gff3",
        output:
          touch = dirGeneidOutputIntrons + "/geneid_introns_run.ok"
        params:
          scripts_dir = scripts_dir,
          prefix = dirMaskedChunks + name_masked,
          out_prediction_geneid_introns = out_prediction_geneid_introns,
    ...
    if config["Parameters"]["run_geneid"]:
      use rule geneid from gene_predictors_workflow with:
        input:
          ...
          geneid_parameters = config["Inputs"]["geneid_parameters"]
        output:
          predictions = out_prediction_geneid
        params:
          geneid_options = geneid_opts,
          path =  config["geneid"]["geneid_path"]
        log:
