...
if config["Inputs"]["genome_lengths"]:
  use rule get_GCcontent from jbrowse_workflow with:
    input:
      genome = genome,
      lengths = config["Inputs"]["genome_lengths"]
    output:
...
if config["Inputs"]["trans_gtf"]:
    cmd = "gawk '$3==\"exon\"' " + config["Inputs"]["trans_gtf"] + "|" + scripts_dir + "gtf_onlyexons_2transcripts.pl | " +\ 
    ...
    " > Stringtie-TACO.gff3;"
    create_browse_dictionary("Stringtie-TACO", config["Inputs"]["trans_gtf"], cmd, "transali", "transcript" )

if config["Parameters"]["run_geneid_introns"] or config["Parameters"]["run_genemark-ET"] or config["Parameters"]["run_augustus_hints"]:
    cmd = "cat " + config["Outputs"]["output_dir"] + "coding_junctions.sorted.gff3 | sed 's/intron/Intron/g' > junctions.gff3;"
    create_browse_dictionary("junctions", config["Outputs"]["output_dir"] + "coding_junctions.sorted.gff3", cmd, "junctions", "Intron" )
...
if config["Parameters"]["run_update"]:
    gff = config["Outputs"]["update_dir"] + config["update"]["project_name"][0] + config["update"]["project_name"][1] + ".gff3" 
    cmd = "grep -v '#' " + gff + " | sed 's/transcript/mRNA/g' > Annotation" + config["update"]["project_name"][1] + ".gff3;"
    create_browse_dictionary("Annotation"+ config["update"]["project_name"][1], gff, cmd, "transcript")

if config["Parameters"]["run_non_coding"]:
    gff = config["Outputs"]["ncRNA_annotation_dir"] + config["update"]["project_name"][0] + "nc" + config["ncRNA_annotation"]["ncRNA_version"] + ".gff3"
...
if config["Inputs"]["RM_gff"]:
    create_browse_dictionary("Repeats", config["Inputs"]["RM_gff"], "", "feature3", "Interspersed_Repeat")
